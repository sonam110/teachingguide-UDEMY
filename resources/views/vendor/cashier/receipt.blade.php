        ...
        <tr valign="top">
            ...
            <td>
                ...
                <table width="100%" class="table" border="0">
                    ...
                    @foreach ($owner->subscriptions as $item)
                        <tr>
                            <td>{{ $item->name }}</td>
                            <td>@if(!empty($item->ends_at)) {{ date('M d, Y', strtotime($item->ends_at)) }} @else {{ $invoice->subscription['billingPeriodEndDate']->format('M d, Y') }} @endif</td>
                            {{-- <td>{{ $invoice->total() }}</td> --}}
                        </tr>
                    @endforeach
                        ...
                        <tr>
                            <td colspan="3">
                                <strong> 
                                    Subscription : {{ $invoice->subscription['billingPeriodStartDate']->format('M d, Y') }} - {{ $invoice->subscription['billingPeriodEndDate']->format('M d, Y') }}
                                </strong>
                            </td>
                        </tr>
