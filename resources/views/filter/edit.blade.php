      ...
      <div class="ibox float-e-margins">
        <div class="ibox-title">
          ...
          <div class="ibox-content">
              <form id="filterform" method="post" action="{{ route('filter.update', ['id' => $filter->id]) }}" class="form-horizontal" enctype="multipart/form-data">
                  @method('PUT')
                  @csrf
                  <div class="row">
                      <div class="col-md-6">
                  ...
                  <div class="row" style="margin-top: 20px;">
                      ...
                      <div class="btn-group pull-right">
                          <button id="updateFilter" type="submit" href="#" class="btn btn-primary">Save Filter</button>
                      </div>
                  </div>
              </form>
          </div>
...
<input type="hidden" id="filterid" value="{{ $filter->id }}" />
@endsection
...
//load filter on document ready
$(document).ready(function(){
  var filterID = $('#filterid').val();

  $.get("/api/filter-get?id="+filterID, function(data, status){
      var ruleset = JSON.parse(data.definition);
      $("#filtername").val(data.name);
      $("#filtertype").val(data.type);
      reloadFilterDef(data.type, ruleset);
  });
});
